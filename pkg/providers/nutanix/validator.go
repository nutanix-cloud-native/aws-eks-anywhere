...
func NewValidator(clientBuilder *ClientBuilder, certValidator crypto.TlsValidator, httpClient *http.Client) *Validator {
	return &Validator{
		clientBuilder: clientBuilder,
...
func (v *Validator) ValidateClusterSpec(ctx context.Context, spec *cluster.Spec, creds credentials.BasicAuthCredential) error {
	...
	client, err := v.clientBuilder.GetNutanixClient(spec.NutanixDatacenter, creds)
	if err != nil {
		return err
	}
